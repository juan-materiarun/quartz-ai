...
export function ReportGenerator({ result, onReset }: ReportGeneratorProps) {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6 pb-8 px-6 animate-in fade-in duration-300">
      {/* Header */}
      <div className="relative flex items-center justify-center py-4 border-b border-[#334155] dark:border-[#334155] border-[#cbd5e1] animate-slide-up-spring">
        <div className="text-center">
          <h1 className="text-2xl font-semibold text-white dark:text-white text-[#0f172a]">{t.report.title}</h1>
          <p className="text-base text-[#94a3b8] dark:text-[#94a3b8] text-[#475569]">{t.report.subtitle}</p>
        </div>
        <div className="absolute right-0 top-1/2 -translate-y-1/2 flex items-center gap-2">
          ...
          <Button 
            ...
            className="h-10 px-4 text-sm font-medium bg-gradient-to-r from-[#38bdf8] to-[#0ea5e9] text-white border-shimmer hover:from-[#38bdf8]/90 hover:to-[#0ea5e9]/90 transition-all duration-300 flex items-center gap-2 shadow-lg"
          >
      ...
      {/* Stats Row */}
      <div className="grid grid-cols-4 gap-4 animate-slide-up-spring">
        <div className="glass-card p-4 rounded-lg">
          <p className="text-sm font-medium text-[#334155] dark:text-[#94a3b8] mb-1">{t.report.stats.passed}</p>
          <p className="text-2xl font-semibold text-[#10b981]">{stats.totalPassed}</p>
        </div>
        <div className="glass-card p-4 rounded-lg">
          <p className="text-sm text-[#94a3b8] mb-1">{t.report.stats.defects}</p>
          <p className="text-2xl font-semibold text-[#ef4444]">{stats.totalDefects}</p>
        </div>
        <div className="glass-card p-4 rounded-lg">
          <p className="text-sm text-[#94a3b8] mb-1">{t.report.stats.critical}</p>
          <p className="text-2xl font-semibold text-[#f59e0b]">{stats.criticalCount}</p>
        </div>
        <div className="glass-card p-4 rounded-lg">
          <p className="text-sm text-[#94a3b8] mb-1">{t.report.stats.score}</p>
          <p className={`text-2xl font-semibold ${getScoreColor(overallScore)}`}>{overallScore}%</p>
        </div>
      </div>

      {/* Integrated Grid: Charts + Score + Passed Tests */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 border-b border-[#334155] pb-6 animate-slide-up-spring">
        {/* Pie Chart */}
        ...
        <div className="glass-card p-4 rounded-lg flex items-center justify-center">
          <div className="text-center">
            ...
            <div className={`text-5xl font-semibold ${getScoreColor(overallScore)} mb-1`}>
              {overallScore}
            </div>
              ...
              result.passedTests.map((test, index) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-white dark:text-white text-[#0f172a]">{test.category}</p>
      ...
      {/* Defects Section */}
      <div className="space-y-4 animate-slide-up-spring">
        <div className="flex items-center justify-between border-b border-[#334155] dark:border-[#334155] border-[#cbd5e1] pb-2">
          <h2 className="text-lg font-semibold text-white dark:text-white text-[#0f172a] flex items-center gap-2">
            {t.report.defects.title}
          </h2>
          <span className="text-sm text-[#94a3b8] dark:text-[#94a3b8] text-[#475569]">
        ...
        <div className="space-y-3">
          {result.defects.length > 0 ? (
            result.defects.map((defect, index) => (
              <div
                key={defect.id}
                className={`glass-card p-4 rounded-lg border ${getPriorityColor(defect.priority)} transition-all duration-300 animate-fade-in`}
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="text-sm font-semibold text-white dark:text-white text-[#0f172a] mb-1">{defect.title}</h3>
                    <p className="text-sm text-[#94a3b8] dark:text-[#94a3b8] text-[#475569] leading-relaxed">{defect.description}</p>
                    {defect.location && (
                      <p className="font-mono text-xs text-[#94a3b8] dark:text-[#94a3b8] text-[#475569] mt-2">{defect.location}</p>
                    )}
                  </div>
                </div>
              </div>
      ...
      <div className="border-t border-[#334155] dark:border-[#334155] border-[#cbd5e1] pt-6 animate-slide-up-spring">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-lg font-semibold text-white dark:text-white text-[#0f172a]">{t.report.testScript}</h3>
          <div className="flex items-center gap-2">
            <Button
              variant="ghost"
              size="sm"
              onClick={copyToClipboard}
              className="h-8 px-3 text-xs text-[#94a3b8] dark:text-[#94a3b8] text-[#475569] hover:text-white dark:hover:text-white hover:text-[#0f172a] hover:bg-[#1e293b] dark:hover:bg-[#1e293b] hover:bg-[#f1f5f9]"
            >
        ...
        {scriptExpanded && (
          <div className="glass-card rounded-lg border border-[#334155] overflow-hidden">
            <pre className="font-mono text-xs p-4 overflow-x-auto max-h-[300px] overflow-y-auto scrollbar-thin">
